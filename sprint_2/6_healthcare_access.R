...
electives <- read_csv(here::here('sprint_2', 'data', "Adm_rates_chrs_chnursing_March-May_countwide.csv"))
...
electives <- electives %>% 
  ...
  filter(grepl("elod_", type2)) %>% 
  mutate(MainICD10Cat = gsub("elod_", "", type2),
         MainICD10Cat = gsub("ICD_", "", MainICD10Cat))
